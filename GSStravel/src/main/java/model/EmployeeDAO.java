...
public class EmployeeDAO implements IEmployeeDAO {
	...
	private static final String Fine_Email = "SELECT emp_Name, emp_Mail FROM Employee WHERE emp_No=?";
	...
	public EmployeeVO selectFineEmail(int no) {
		...
		try (Connection conn = ds.getConnection();
			...
			if (rset.next()) {
				...
				result.setEmp_Name(rset.getString("emp_Name"));
				result.setEmp_Mail(rset.getString("emp_Mail"));
			}
	...
	public void updateYear(int year) {
		try (Connection conn = ds.getConnection(); PreparedStatement stmt = conn.prepareStatement(updateYear);) {
			stmt.setInt(1, year);
			stmt.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public String selectEmp_Name(String emp_No) {
		String name = null;
		try (Connection conn = ds.getConnection(); PreparedStatement stmt = conn.prepareStatement(selectEmp_Name);) {
	...
	public Integer selectEmp_Sub(String emp_No) {
		int Emp_Sub = 0;
		try (Connection conn = ds.getConnection(); PreparedStatement stmt = conn.prepareStatement(selectEmp_Sub);) {
			stmt.setString(1, emp_No);
			ResultSet rset = stmt.executeQuery();
			while (rset.next()) {
	...
	public void updateEmp_Sub(boolean emp_Sub, String emp_No) {
		try (Connection conn = ds.getConnection(); PreparedStatement stmt = conn.prepareStatement(updateEmp_Sub);) {
	...
	public void updateEmp_SubTra(String tra_No, String emp_No) {
		try (Connection conn = ds.getConnection(); PreparedStatement stmt = conn.prepareStatement(updateEmp_SubTra);) {
