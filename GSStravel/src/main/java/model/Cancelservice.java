...
public class Cancelservice {
	...
	public List<Long> detail(Integer emp_No,long tra_No){
		List<Long> detail=new ArrayList<>();
		EmployeeVO employeeVo = employeeService.select(emp_No.toString());					
		java.sql.Date hireDate =employeeVo.getEmp_HireDate();//員工到職日
		...
		TotalAmountVO totalAmountVo = totalAmountService.select(emp_No.toString(),""+tra_No);
		long ta_Money =(long)totalAmountVo.getTa_Money();
		
		//如果點選取消的旅遊是贈正在使用補助金,去算補助金的額度,如果不是補助金為0元
		if(employeeVo.getEmp_SubTra().equals(""+tra_No)){
