					...
					<tr>
						...
						<c:forEach var="j" varStatus="statusJ" begin="0" end="${countI}">
							<c:if test="${statusJ.count==1}">
								<td>報名截止日 ～ ${totalDays[0][j]}<br>
								<fmt:formatNumber value="${iSelect[0].item_Money}"
							...
							<c:if test="${statusJ.count!=1}">
								<td>${afterDay[0][j-1]}～ <fmt:formatNumber
										value="${totalDays[0][j]}" groupingUsed="true" type="currency"
