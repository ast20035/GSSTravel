						...
						<tr>
							...
							<c:forEach var="j" varStatus="statusJ" begin="0" end="${countI}">
								<c:if test="${statusJ.count==1}">
									<td>報名截止日 ～ ${totalDays[i][j]}<br><fmt:formatNumber value="${iSelect[i].item_Money}" groupingUsed="true" type="currency" maxFractionDigits="0"/> * ${fSelect[j].fine_Per}% ＝ <fmt:formatNumber value="${iSelect[i].item_Money*fSelect[j].fine_Per/100}" groupingUsed="true" type="currency" maxFractionDigits="0"/></td>
								</c:if>
								<c:if test="${statusJ.count!=1}">
									<td>${afterDay[i][j-1]} ～ ${totalDays[i][j]}<br><fmt:formatNumber value="${iSelect[i].item_Money}" groupingUsed="true" type="currency" maxFractionDigits="0"/> * ${fSelect[j].fine_Per}% ＝ <fmt:formatNumber value="${iSelect[i].item_Money*fSelect[j].fine_Per/100}" groupingUsed="true" type="currency" maxFractionDigits="0"/></td>
								</c:if>
							</c:forEach>
							<td>${totalDays[i][countI+1]}<br><fmt:formatNumber value="${iSelect[i].item_Money}" groupingUsed="true" type="currency" maxFractionDigits="0"/></td>
						</tr>
					</c:forEach>
				</table>
			</c:if>
